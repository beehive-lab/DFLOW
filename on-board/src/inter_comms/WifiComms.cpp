...
int send(char data[1024]) {
    ...
    if (status == -1) {
        ...
        if (connect_status == -1) {
            cerr<<"Error connecting to the socket";
            exit(1);
        }
...
int listen_to_connection(int port) {
    ...
    if (getaddrinfo(nullptr, port_char, &hints, &server_info) != 0) {
        cerr<<"Error retrieving information from server";
        exit(1);
    }
    ...
    if (bind_status == -1) {
        cerr<<"Error binding the address to the socket";
        exit(1);
    }
...
//    }
    ...
    if (listen_status == -1) {
        cerr<<"Error listening to the port";
        exit(1);
    }
